...
// | 文件说明：微信php类库  
// +----------------------------------------------------------------------
// | Contributors: wuwu <15093565100@163.com> qq群 9498810
// +----------------------------------------------------------------------
...
class Wechat
{
	...
	*/
	 ...
	 * @DateTime: 2017-07-02T22:46:48+0800
	 * @since:    1.0
	 * @return    json 
	 */
    ...
    /**
     * [tagsUserList 获取标签下粉丝列表]
     * @Author:   wuwu<15093565100@163.com>
     * @DateTime: 2017-07-07T16:06:47+0800
     * @since:    1.0
     * @param     string                    $tagid [标签id]
     * @param     string                    $next_openid [第一个拉取的OPENID，不填默认从头开始拉取]
     * @return    [type]                   [description]
     */
    public function tagsUserList($tagid,$next_openid=""){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
                'tagid'=>$tagid,
    ...
    /**
     * [tagsBatchTagging 批量为用户打标签]
     * @Author:   wuwu<15093565100@qq.com>
     * @DateTime: 2017-07-07T16:47:48+0800
     * @since:    1.0
     * @param     array                    $openidlist [OPENID数组 索引数组]
     * @param     [type]                   $tagid      [标签id]
     * @return    [type]                               [description]
     */
    public function tagsBatchTagging(array $openidlist,$tagid){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
     ...
     * @return    [type]                               [description]
     */
    public function tagsBatchunTagging(array $openidlist,$tagid){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
                'openid_list'=>$openidlist,
                'tagid'=>$tagid
    ...
    /**
     * [tagsGetidlist 获取用户身上的标签列表]
     * @Author:   wuwu<15093565100@qq.com>
     * @DateTime: 2017-07-07T16:32:28+0800
     * @since:    1.0
	 ...
	 * [tagsCreate 创建用户标签]
	 * @Author:   wuwu<15093565100@163.com>
	 * @DateTime: 2017-07-02T22:13:39+0800
	 * @since:    1.0
	 ...
	 * @return    boolean
	 */
	public function tagsUpdate($tagId,$name){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
				'tag'=>array(
					'id'  =>$tagId,
	 ...
	 * @return    boolean
	 */
	public function tagsDelete($tagId){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
				'tag'=>array(
					'id'  =>$tagId
