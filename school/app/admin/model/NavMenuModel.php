...
class NavMenuModel extends Model
{
     ...
     * @return array
     */
    public function navMenusTreeArray($navId = 0, $maxLevel = 0,$field='*')
    {
        if (empty($navId)) {
            $navId = Db::name('nav')->where('is_main', 1)->value('id');
        }
        $navMenus     = $this->where('nav_id', $navId)->field($field)->where('status', 1)->order('list_order ASC')->select()->toArray();
        $navMenusTree = [];
        if (!empty($navMenus)) {
            $tree = new Tree();
